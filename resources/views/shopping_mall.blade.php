                ...
                var sources = function () {
                    ...
                    @foreach($products as $product)
                        result.push('<div class="Product shadow " data-index="' +
                            ...
                            '{{route("shopping_mall_detail", ["key" =>$product->pic_url , "page_id"=>$page_id ] )}}' + '\';"><div class="Product_Img"  style="background-image: url(' +
                            '{{$product->pic_url }}' + ')"></div><div class="Product_Content pt-3"><P class="text-truncate">商品名稱： ' +
                            '{{$product->goods_name}}' + '</P><P class="text-truncate">商品價格：<a class=" text-danger currencyField">' +
                    ...
                    @endforeach
                    var space = result.length%4;
                    space =  4 - space;
                    for(var i=0; i<space ;i++ ){
                        result.push('<div class="Product shadow invisible d-none d-sm-none d-md-inline-block " data-index="' +
                            '' + '" onclick="location.href=\'' +
                            '' + '\';"><div class="Product_Img"  style="background-image: url(' +
                            '' + ')"></div><div class="Product_Content pt-3"><P class="text-truncate">商品名稱： ' +
                            '' + '</P><P class="text-truncate">商品價格：<a class=" text-danger currencyField">' +
                            '' + '</a></P class="text-truncate"><small class="text-black-50">剩餘數量：' +
                            '' + '</small><br><small class="text-black-50 text-truncate">銷售數量：' +
                            '' + '</small></div></div>');
                    }
                    return result;
                }();
